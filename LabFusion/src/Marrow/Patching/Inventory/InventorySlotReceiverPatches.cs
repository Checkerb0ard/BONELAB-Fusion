...
public class InventorySlotReceiverPatches
{
    ...
    private static void OnDropWeapon(InventorySlotReceiver __instance, Hand hand = null)
    {
        ...
        var data = new InventorySlotDropData() { SlotEntityID = slotEntity.ID, GrabberID = PlayerIDManager.LocalSmallID.Value, SlotIndex = index.Value, Handedness = handedness };

        ClientManager.RelayModule<InventorySlotDropMessage, InventorySlotDropData>(data, CommonMessageRoutes.ReliableToOtherClients);
    }
    ...
    public static void OnHandDropPostfix(InventorySlotReceiver __instance, IGrippable host)
    {
        ...
        ClientManager.RelayModule<InventorySlotInsertMessage, InventorySlotInsertData>(data, CommonMessageRoutes.ReliableToOtherClients);
    }
